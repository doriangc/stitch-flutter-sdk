...
import '../../internal/net/stitch_auth_doc_request.dart'
    show StitchAuthDocRequest;
import '../../internal/net/stitch_auth_request.dart'
    show StitchAuthRequest, StitchAuthRequestBuilder;
import '../../internal/net/stitch_doc_request.dart'
    show StitchDocRequestBuilder;
...
import '../providers/anonymous/anonymous_auth_provider.dart'
    show AnonymousAuthProvider;
import '../providers/internal/stitch_auth_response_credential.dart'
    show StitchAuthResponseCredential;
...
import './models/store_auth_info.dart'
    show
...
import 'stitch_auth_request_client.dart' show StitchAuthRequestClient;
import '../../stitch_client_exception.dart';
...
abstract class CoreStitchAuth<TStitchUser extends CoreStitchUser>
  ...
  bool get hasDeviceId => (activeUserAuthInfo.deviceId != null &&
  ...
  CoreStitchAuth(this.requestClient, this.authRoutes, this.storage,
  ...
  Future<void> initProcess() async {
    ...
    activeUserAuthInfo =
        activeUserAuthInfo == null ? AuthInfo.empty() : activeUserAuthInfo;
  ...
  Future<T> doAuthenticatedRequestWithDecoder<T>(StitchAuthRequest stitchReq,
  ...
  Future<EventStream> openAuthenticatedEventStream(StitchAuthRequest stitchReq,
    ...
    try {
      ...
      if (partialInfo.accessToken != null && user is CoreStitchUserImpl) {
        Map<String, dynamic> userData =
            JWT.fromEncoded(partialInfo.accessToken).userData;
        (this.user as CoreStitchUserImpl).customData =
            userData == null ? {} : userData;
      }
    } catch (err) {
      throw new StitchDecodingException(err);
    }

    try {
      writeActiveUserAuthInfoToStorage(activeUserAuthInfo, this.storage);

      this.allUsersAuthInfo[this.activeUserAuthInfo.userId] =
      ...
      // );
      throw StitchClientException(
          StitchClientExceptionCode.couldNotPersistAuthInfo);
    }
  }
  ...
  TStitchUser switchToUserWithId(String userId) {
    ...
    if (authInfo == null) {
      ...
      throw StitchClientException(StitchClientExceptionCode.userNotFound);
    }
    if (!authInfo.isLoggedIn) {
      throw StitchClientException(StitchClientExceptionCode.userNotLoggedIn);
    ...
    this.dispatchAuthEvent(ActiveUserChanged<TStitchUser>(
        currentActiveUser: this.currentUser, previousActiveUser: previousUser));
  ...
  Future<TStitchUser> loginWithCredentialInternal(
    ...
    if (credential is StitchAuthResponseCredential) {
      TStitchUser user = await _processLogin(
          credential, credential.authInfo, credential.asLink);
  ...
  Future<TStitchUser> linkUserWithCredentialInternal(
      CoreStitchUser user, StitchCredential credential) {
    if (this.currentUser != null && user.id != this.currentUser.id) {
      ...
      throw StitchClientException(StitchClientExceptionCode.userNoLongerValid);
    }
  ...
  Future<void> removeUserWithIdInternal(String userId) async {
    ...
    Function removeBlock = () {
      ...
      this.onAuthEvent();
      this.dispatchAuthEvent(UserRemoved(removedUser));
    };
  ...
  StitchRequest prepareAuthRequest(
    ...
    if (stitchReq.useRefreshToken != null && stitchReq.useRefreshToken) {
      newHeaders[Headers.AUTHORIZATION] =
          Headers.getAuthorizationBearer(authInfo.refreshToken);
    } else {
      newHeaders[Headers.AUTHORIZATION] =
          Headers.getAuthorizationBearer(authInfo.accessToken);
    }
    newReq.withHeaders(newHeaders);
    return newReq.build();
  }

  Future<EventStream> _handleAuthFailureForEventStream(StitchAuthRequest req,
      {open = true}) async {
    ...
    return this.openAuthenticatedEventStream(
        req.builder().withShouldRefreshOnFailure(false).build(), open);
  }
  ...
  Future<void> tryRefreshAccessToken(num reqStartedAt) async {
    ...
    if (!this.isLoggedIn) {
      ...
      throw StitchClientException(
          StitchClientExceptionCode.loggedOutDuringRequest);
    }
  ...
  Future<TStitchUser> _doLogin(
    ...
    if (asLinkRequest) {
      ...
      this.dispatchAuthEvent(ActiveUserChanged(
          currentActiveUser: user, previousActiveUser: previousActiveUser));
    }
  ...
  Future<Response> _doLoginRequest(
    ...
    // TODO: optimize
    credential.material.forEach((key, value) {
      // naive clone into 'dynamic' type
      material[key] = value;
    });
    ...
    StitchAuthDocRequest linkRequest =
        StitchAuthDocRequest(reqBuilder.build(), reqBuilder.document);
  ...
  Future<TStitchUser> _processLogin(StitchCredential credential,
      AuthInfo newAuthInfo, bool asLinkRequest) async {
    ...
    newAuthInfo = this.activeUserAuthInfo.merge(new AuthInfo(
        newAuthInfo.userId,
    ...
    try {
      ...
      if (oldActiveUserInfo.hasUser) {
        this.allUsersAuthInfo[oldActiveUserInfo.userId] =
            oldActiveUserInfo.withNewAuthActivityTime();
      }

      // Finally set the info and user
      newAuthInfo = newAuthInfo.merge(new AuthInfo(
          newAuthInfo.userId,
          newAuthInfo.deviceId,
          newAuthInfo.accessToken,
          newAuthInfo.refreshToken,
      ...
      bool newUserAdded =
          !this.allUsersAuthInfo.containsKey(newAuthInfo.userId);
      ...
      try {
        ...
        if (newAuthInfo.userId != oldActiveUserInfo.userId &&
            newAuthInfo.userId != null) {
      ...
      if (asLinkRequest || oldActiveUserInfo.hasUser) {
        ...
        // Otherwise this was a normal login request, so log the user out
        _clearActiveUserAuth();
      }
  ...
  Future<TStitchUser> _processLoginResponse(
      StitchCredential credential, Response response, bool asLinkRequest) {
    try {
      if (response == null) {
        throw StitchServiceException(
            'the login response could not be processed for credential: $credential; response was undefined');
      ...
      return _processLogin(credential,
          ApiAuthInfo.fromJSON(json.decode(response.body)), asLinkRequest);
    } catch (err) {
      throw new StitchRequestException(
          '${StitchRequestExceptionCode.DECODING_ERROR} | $err');
  ...
  _clearUserAuthTokens(String userId) {
    ...
    try {
      ...
      if (this.activeUserAuthInfo.hasUser &&
          activeUserAuthInfo.userId == userId) {
        wasActiveUser = true;
      ...
      if (loggedOutUser != null) {
        ...
        if (wasActiveUser) {
          this.dispatchAuthEvent(ActiveUserChanged(
              currentActiveUser: null, previousActiveUser: loggedOutUser));
        }
      }
    } catch (err) {
      throw StitchClientException(
